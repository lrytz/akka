...
//   case class TestContext(queue: BoundedBlockingQueue[String], events: mutable.ArrayDeque[QueueEvent], notEmpty: TestCondition, notFull: TestCondition, lock: ReentrantLock, backingQueue: util.Queue[String])

//   /**
//    * Backing queue that records all poll and offer calls in `events`
//    */
//   class TestBackingQueue(events: mutable.ArrayDeque[QueueEvent])
...
//    */
//   class TestCondition(events: mutable.ArrayDeque[QueueEvent], condition: Condition, signalEvent: QueueEvent, awaitEvent: QueueEvent)
//     extends Condition {
...
//     val events: mutable.ArrayDeque[QueueEvent] = new mutable.ArrayDeque()
